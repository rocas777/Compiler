        ...
        for (JmmNode jmmNode : children) {
            ...
            switch (kind) {
                ...
                case "VariableName": {
                    ...
                    Report report = SearchHelper.CheckIfInteger(jmmNode.get("name"), methodName, table, "Variable " + jmmNode.get("name") + " is not an int", Integer.parseInt(jmmNode.get("line")), Integer.parseInt(jmmNode.get("column")));
                    if (report != null) reports.add(report);
                    break;
                }
                default: {
